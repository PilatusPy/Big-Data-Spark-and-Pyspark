...
SELECT 
    ...
    SUM(CASE WHEN marital_status = 'Never-married' THEN 1 ELSE 0 END) AS never_married_count,
    ROUND((SUM(CASE WHEN marital_status = 'Never-married' THEN 1 ELSE 0 END) / COUNT(*)), 2) AS bachelor_rate
FROM 
    adult
GROUP BY 
    education
...
df_result = df_adult.groupBy('education').agg(
    ...
    sum(when(col('marital_status') == 'Never-married', 1).otherwise(0)).alias('never_married_count')
).withColumn(
    'bachelor_rate',  
    round((col('never_married_count') / col('total_count')), 2)
).orderBy(
    col('bachelor_rate').desc()
)
df_result.show(1)
...
# look at best params from the CV
print("Best MaxDepth: {}".format(bestModel._java_obj.getMaxDepth()))
print("Best MaxBins: {}".format(bestModel._java_obj.getMaxBins()))
print("Best MaxIter: {}".format(bestModel._java_obj.getMaxIter()))
...
#extracting feature names based on the metadata
feature_attrs = dataset.schema["features"].metadata["ml_attr"]["attrs"]
feature_names = [f["name"] for f in (feature_attrs["binary"] + feature_attrs["numeric"])]

# Coefficients from the Logistic Regression Model
coefficients = lrModel.coefficients.toArray()

# List of tuples containing feature names and coefficients
lr_feature_coefficients_list = [(name, coef) for name, coef in zip(feature_names, coefficients)]

#dataframe
lrFeatureCoefficients = pd.DataFrame(lr_feature_coefficients_list, columns=['feature_name', 'coefficient'])

pd.options.display.float_format = '{:.4f}'.format

print(lrFeatureCoefficients.sort_values(by='coefficient', key=lambda x: x.abs(), ascending=False))


# COMMAND ----------

#transforming data
transformed_data = gbCvModel.bestModel.transform(dataset)

#attrs needs to get indexes and names
attrs = [(i, attr["name"]) for i, attr in enumerate(transformed_data.schema["features"].metadata["ml_attr"]["attrs"]["binary"] + transformed_data.schema["features"].metadata["ml_attr"]["attrs"]["numeric"])]

#dataframe
gbCvFeatureImportance = pd.DataFrame(
    [(name, gbCvModel.bestModel.featureImportances[idx]) for idx, name in attrs], 
...
rf_feature_importance_list = [(name, importance) for name, importance in zip(feature_names, feature_importances)]

rfFeatureImportance = pd.DataFrame(rf_feature_importance_list, columns=['feature_name', 'feature_importance'])
